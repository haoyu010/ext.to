...
- **种子链接永远可用、而且看得见** — 模板里用 `<a href="{torrent_url}">{torrent_text}</a>` 会直接印出链接本身（形如 `magnet:?xt=urn:btih:449ffc…`），转发到别处也不丢；`{torrent_url}` 优先填磁力链接，取不到磁力时自动退回详情页链接，不会发出去一个点不开的死链接
...
**为什么印短链而不是整条磁力。** 站点给出的磁力链接带二十多个 tracker，实测长度 331 ~ 1188 字符；Telegram 的 1024 字符上限只算**显示出来的文字**，链接的去向（`href`）不计 —— 这条是在真实频道上量出来的：`<a href="整条磁力">种子链接</a>` 能发出去，把同一条磁力当文字印出来就回 `message caption is too long`。所以 `{torrent_text}` 只保留定位种子必需的 info hash（`magnet:?xt=urn:btih:449ffc…`，60 字符），丢掉的那串 tracker 客户端会通过 DHT 与 PEX 自己找回来；`{torrent_url}` 里仍是**完整**链接，只作 `href` 用。`{magnet}` 印的是完整磁力，请只放进 `href`，当正文文字会被 Telegram 拒绝。
