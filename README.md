...
- **种子链接永远可用、而且看得见** — 模板里用 `<a href="{torrent_url}">{torrent_text}</a>` 会把**完整磁力链接**印出来（连 tracker 一起），转发到别处也不丢；`{torrent_url}` 优先填磁力链接，取不到磁力时自动退回详情页链接，不会发出去一个点不开的死链接
...
| `{torrent_url}` `{torrent_text}` | **推荐**。`{torrent_url}` 是链接的去向，优先填磁力链接，取不到磁力时退回详情页链接，所以永远可用；`{torrent_text}` 是显示出来的文字，直接印出链接本身，转发到别处也不会丢。写成 `<a href="{torrent_url}">{torrent_text}</a>` |
| `{torrent_label}` | 旧模板用的文字版，渲染成「种子链接」或「详情页」；仍然可用，但转发后只剩这四个字 |
...
**为什么印的是整条磁力。** 磁力链接里的 tracker 不是装饰：只有 info hash 的链接，BT 客户端靠 DHT 还能开，但网盘离线下载不行 —— 它拿到 hash 找不到任何 peer，只会显示一串 hash 当成文件名。所以 `{torrent_text}` 印出**完整**链接，tracker 一个都不丢。

**放不下的时候按这个顺序让位。** 站点给出的磁力实测 331 ~ 1188 字符，Telegram 的图片说明上限是 **1024 个字符，只算看得见的文字**（`href` 里的去向不计，`<b>` 这类标签也不计，`&amp;` 算 1 个而不是 5 个 —— 这几条都在真实频道上量过：1024 个字符能发，1025 个回 `message caption is too long`）。一整套 caption 未必塞得下 1188 字符，所以渲染器会按顺序让步，每步都重新量一次：

1. **先缩简介**：`{tmdb_overview}` 从 320 字起，每 40 字一档往下让，直到整条链接放得下；让到 0 就整行去掉，不会留下 `简介：…` 这种只剩省略号的行。
2. **再缩链接**：简介全让完还不够，才从链接尾部丢 tracker，一次丢一整个参数，绝不切在 URL 中间；`xt=urn:btih:...` 这个 info hash **永远保留**，没有它就不知道是哪个种子了。
3. **最后兜底**：万一连这些都还不够（例如标题本身超长），会直接截断整条说明，宁可发一条短的，也不发一条 Telegram 直接拒收的 —— 被拒的消息在记录里算永久失败，不会重试，等于这条种子丢了。

`{torrent_url}` 里始终是**完整**链接，所以就算文字被缩过，点进去照样带全部 tracker。`{magnet}` 印的也是完整磁力，但它超过 1024 字符、带海报时会被 Telegram 拒收，请只放进 `href`。
...
**升级时会自动换模板。** 旧版本的默认模板与 TMDB 模板只链详情页、且从未用到 `{magnet}`，所以那样装的实例没有途径知道有种子链接这回事；1.3.0 / 1.3.1 的预设则链了种子、却把文字写死成「种子链接」，转发后只剩四个字。启动时会按**逐字节**比对把这些旧预设换成新预设（1.2.x 的两套会一步换到最新版）；只要改动过（哪怕只加了一行），就完全保持原样不动 —— 静默改写别人自己写的正文，比留着旧模板更糟。
