...
- **种子链接永远可用、而且看得见** — 模板里直接用 `{torrent_text}` 会把**完整磁力链接**印出来（连 tracker 一起），转发到别处也不丢；取不到磁力时自动退回详情页链接，不会发出去一个点不开的死链接
- **磁力链接可一键复制（可选）** — 可以开启一个「复制磁力链接」按钮，点一下就把链接放进剪贴板。之所以不是「点击直达」的超链接：**Telegram 不接受 `magnet:` 开头的超链接**（写成锚点会被静默丢掉，写成 entity 会被拒）。这个按钮**默认关闭**，因为它有实测代价：带按钮的消息会被频道的**网页预览**（`t.me/s/频道名`）拒之门外，只显示「Please open Telegram to view this post」而看不到正文；同一条正文不带按钮即可正常显示
- **磁力链接会被修正成合法 URL** — 站点把发布名原样塞进 `dn` 参数，里面带空格和方括号，这样的 URI 是非法的：严格解析的下载器读到第一个空格就停。渲染前会把 `dn` 里的非法字符转义，实测能解析出完整发布名
...
| `with_copy_button` | 消息下方加「复制磁力链接」按钮。**默认 `false`**：带按钮的消息会被 `t.me/s/` 网页预览拒绝渲染，见下文 |
| `silent` | 静默推送，不响铃 |
...
| `{url}` `{magnet}` `{id}` | 详情页链接、磁力链接（已修正非法字符）、种子 ID |
| `{torrent_url}` `{torrent_text}` | **推荐**。`{torrent_url}` 是链接的去向，优先填磁力链接，取不到磁力时退回详情页链接，所以永远可用，但它只适合放进 `href`；要显示就用 `{torrent_text}`，直接写它即可，**不要**再套 `<a>` |
...
> **不要把 `{torrent_text}` 套进 `<a>` 里。** Telegram 不接受 `magnet:` 开头的超链接：写成 HTML 锚点会被**静默丢掉**（消息照发，只是没有链接），写成 `entities` 会被拒（`Wrong port number specified in the URL`），按钮 `url` 字段同样被拒。所以套上锚点只会让正文看起来像能点，其实点不开。`{torrent_text}` 自己会处理显示：
>
> - 有磁力时，链接套在 `<code>` 里 —— Telegram 客户端点一下即可复制，同时挡住磁力里夹杂的 `UIndex.org` 这类字样被自动识别成蓝色链接（实测：不套 `<code>` 时，那个域名会被单独识别成一个链接）；
> - 取不到磁力时退回详情页，此时是**裸链接** —— 实测把 `<code>` 套进锚点会连锚点一起失效（`text_link` 变成 `code`），而裸 https 链接 Telegram 自己就会识别。
>
> 想要「一键拿到磁力」，靠的是消息下方那个复制按钮，不是锚点。
...
**站点给出的磁力是非法的，本程序会修正。** ext.to 把发布名原样写进磁力的 `dn` 参数，而发布名带空格、方括号：`dn=The Taking of Tiger Mountain 2014 m720p BluRay x264-GeneMige [ UIndex.org ]`。带裸空格的 URI 是非法的，严格解析的客户端读到第一个空格就停 —— 于是链接「看着像对的，实际用不了」。渲染前会把参数值里的非法字符 percent-encode（`&`、`=` 这类分隔符不动，否则会并掉参数），实测修正后能用标准解析器还原出完整发布名。另外站点偶尔会吐出一个没有名字的碎片参数（发布名里带裸 `&` 时），这种会被丢掉。

**复制按钮与正文是两份不同的链接。** 正文受 1024 字符说明上限约束；复制按钮受 Telegram 自己的 **256 字符**上限约束（实测 256 能收、257 回 `BUTTON_COPY_TEXT_INVALID`）。所以按钮固定带上 `xt=urn:btih:...` 和尽量多的 tracker，一次一个整参数地截，绝不切在 URL 中间。按钮被拒时不会丢消息：转发器会去掉按钮把正文照常发出。

> **复制按钮为什么默认关闭。** 实测代价：**带按钮的消息无法被频道的网页预览渲染。** 在 `t.me/s/<频道名>` 这个公开预览页上，带按钮的消息整条变成 `text_not_supported_wrap`，只显示「Please open Telegram to view this post」，正文里的磁力、标题、简介一个字都看不到；把同一条正文去掉按钮再发一次，预览页就能完整显示。这条是在真实频道上对比两条消息得出的，不是推测。
>
> Telegram 客户端内部不受影响 —— 按钮正常显示、正文正常显示。所以这是个取舍：只能通过网页预览看频道的人会因此看不到内容。正文里的磁力本来点一下就能复制（`<code>` 的作用），所以不需要就别开。开关在「Telegram 推送 → 发送选项 → 复制磁力按钮」。
...
正文里的链接被缩过，说明**整段简介都让光了**还是放不下，此时只能牺牲末尾的 tracker。`{magnet}` 印的也是完整磁力，但它超过 1024 字符、带海报时会被 Telegram 拒收，不要直接放进正文做显示。
...
**升级时会自动换模板。** 旧版本的默认模板与 TMDB 模板只链详情页、且从未用到 `{magnet}`，所以那样装的实例没有途径知道有种子链接这回事；1.3.0 / 1.3.1 的预设则链了种子、却把文字写死成「种子链接」，转发后只剩四个字；1.3.2 / 1.3.3 的预设给磁力套了 `<a>`，而 Telegram 对磁力锚点照丢不误，等于留了一行点不开的链接。启动时会按**逐字节**比对把这些旧预设换成新预设（1.2.x 的两套会一步换到最新版）；只要改动过（哪怕只加了一行），就完全保持原样不动 —— 静默改写别人自己写的正文，比留着旧模板更糟。
